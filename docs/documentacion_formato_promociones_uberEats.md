---
layout: default
title: "Formato de Promociones y Descuentos – UberEats"
subtitle: "Instructivo de configuración de promociones y descuentos en UberEats"
---

## **1. Conceptos**

En UberEats hay dos mecanismos distintos que pueden parecer similares pero funcionan de forma completamente diferente:
...
Un descuento es una reducción de precio que UberEats aplica sobre un producto existente o agrupación de ítems: simplemente el producto vale menos.

### **2.1. Características**

- El producto se carga de forma normal en el catálogo, **sin cambios en el nombre ni en el SKU**.
- El descuento se configura desde el panel de promociones de UberEats y llega en el pedido de forma separada (no se codifica dentro del SKU).
- El descuento llega por producto y también como total del pedido.
...
Un `1/2 kg de Helado` (SKU `65`) con 20% Off configurado por UberEats:

| Elemento | Nombre en UberEats | SKU | Descuento |
| :--- | :--- | :--- | :--- |
| Producto con descuento | 1/2 kg de Helado | **65** | 20% Off (configurado en UberEats) |
...
<div class="note">
  <p><strong>Diferencia clave con Rappi / MercadoPago:</strong> en UberEats <strong>no se usa el formato de SKU con guion</strong> (<code>64-A</code>, <code>64-A-99999</code>). La relación entre la cabecera, los contenedores y los sabores la provee la <strong>estructura nativa de modificadores de UberEats</strong> (grupos de modificadores → opciones). El sabor se identifica porque su SKU es <strong><code>99999</code></strong> (o porque su código externo no es numérico).</p>
</div>
...
| Elemento | Nombre en UberEats | SKU a cargar |
| :--- | :--- | :--- |
...
A diferencia de Rappi / MercadoPago, **no se usa un formato especial de SKU con guion**. En UberEats cada contenedor (tamaño) es un **grupo de modificadores** y los sabores son sus **opciones**.

#### **Cómo se carga en el catálogo de UberEats**
...
La relación contenedor → sabores la da la **estructura nativa de UberEats**: cada sabor está literalmente anidado dentro de su grupo. No hace falta ninguna letra ni prefijo de SKU.
...
El SKU `99999` es la marca que le indica al sistema que la opción es un **sabor** (texto descriptivo) y no un producto con línea propia. El sistema también trata como sabor cualquier opción cuyo código externo **no sea numérico**. En ambos casos, el sistema agrupa automáticamente esos sabores bajo la línea de su contenedor.
...
**Configuración en el catálogo de UberEats:**

A diferencia de Rappi/MercadoPago, en UberEats los sabores se cargan **anidados dentro de su grupo** (estructura jerárquica nativa). La relación contenedor → sabor no la asignamos por SKU, la da el propio anidamiento del catálogo.

```
Promo 1 Kilo + 1/2 Kilo                       ← Producto cabecera (title empieza con "Promo")   SKU: 2500
