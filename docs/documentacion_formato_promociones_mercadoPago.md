---
layout: default
title: "Formato de Promociones y Descuentos – MercadoPago"
subtitle: "Instructivo de configuración de promociones y descuentos en MercadoPago"
---

## **1. Conceptos**

En MercadoPago hay dos mecanismos distintos que pueden parecer similares pero funcionan de forma completamente diferente:
...
Un descuento es una reducción de precio que MercadoPago aplica sobre un producto existente o agrupación de ítems: simplemente el producto vale menos.

### **2.1. Características**

- El producto se carga de forma normal en el catálogo, **sin cambios en el nombre ni en el SKU**.
- El descuento se configura desde el panel de promociones de MercadoPago y llega en el pedido de forma separada (no se codifica dentro del SKU).
- El descuento puede tener dos orígenes: la porción que aporta el **comercio** (`seller`) y la que aporta **MercadoPago** (`meli`). El sistema toma la porción del comercio (`seller`) para reflejar el descuento del comercio en el pedido.
...
Un `1/2 kg de Helado` (SKU `65`) con 20% Off configurado por MercadoPago:

| Elemento | Nombre en MercadoPago | SKU | Descuento |
| :--- | :--- | :--- | :--- |
| Producto con descuento | 1/2 kg de Helado | **65** | 20% Off (configurado en MercadoPago) |
...
El nombre de **toda promo debe comenzar con la palabra `Promo`**. El sistema reconoce la promo cuando el nombre del ítem comienza con `Promo`; la detección **no distingue mayúsculas de minúsculas** (el sistema normaliza el texto), pero se recomienda escribirla con `P` mayúscula. Esta es la única forma en que el sistema la reconoce como una agrupación de ítems.
...
La cabecera lleva su SKU numérico de promoción. Los productos dentro de la promo (cargados como modificadores) también llevan su SKU numérico propio del producto, es decir exactamente igual a si se vendieran solos.

**Ejemplo — Promo con dos potes:**

| Elemento | Nombre en MercadoPago | SKU a cargar |
| :--- | :--- | :--- |
...
**Configuración en el catálogo de MercadoPago:**

En MercadoPago todos los modificadores se crean **al mismo nivel**, sin anidamiento. La relación padre-hijo no es una jerarquía visual del catálogo — la asignamos nosotros al darle a cada SKU su formato específico.
...
<div class="note">
  <p>Los 4 modificadores se cargan al mismo nivel en MercadoPago. El <code>min/max</code> de los modificadores <strong>3</strong> y <strong>4</strong> aplica al modificador de sabores completo (cuántos sabores puede elegir el cliente), no a cada sabor por separado. Dentro de cada modificador de sabores, todas las opciones comparten el mismo SKU; su prefijo (<code>64-A</code> / <code>65-A</code>) es lo que las vincula a su tamaño.</p>
</div>
